...
def assert_dimension_size(
    ...
    expected_dimension_size: tuple[int, ...] | int,
) -> None:
    assert_tensor(tensor_name, tensor)

    if isinstance(dimension, int):
        assert isinstance(expected_dimension_size, int), \
            ("Invalid type for 'expected_dimension_size' (expected 'int', "
             f"got {type(expected_dimension_size)})")

        assert tensor.shape[dimension] == expected_dimension_size, \
            (f"Invalid size for dimension {dimension} of {tensor_name} "
                f"(expected {expected_dimension_size}, got {tensor.shape[dimension]})")
    elif isinstance(dimension, tuple):
        assert isinstance(expected_dimension_size, tuple), \
            ("Invalid type for 'expected_dimension_size' (expected 'tuple', "
                f"got {type(expected_dimension_size)})")

        assert len(dimension) == len(expected_dimension_size), \
            (f"Mismatched length between 'dimension' and 'expected_dimension_size' "
                f"({len(dimension)} vs. {len(expected_dimension_size)})")

        for dim, size in zip(dimension, expected_dimension_size):
            assert tensor.shape[dim] == size, \
                (f"Invalid size for dimension {dim} of {tensor_name} "
                    f"(expected {size}, got {tensor.shape[dim]})")
    else:
        raise ValueError(
            f"Invalid type for 'dimension' (expected 'int' or 'tuple', got {type(dimension)})")
...
class PositionalEncoding(nn.Module):
    ...
    def forward(
        ...
        assert_same_partial_shape('X', X, 'P', P, dims=(-2, -1))
...
class DotProductAttention(nn.Module):
    ...
    def forward(
        ...
        assert_same_partial_shape('queries', queries, 'keys', keys, dims=2)

        assert_dimension('valid_lens', valid_lens, (1, 2))
        if valid_lens.dim() == 1:
            assert_same_partial_shape('valid_lens', valid_lens, 'keys', keys, dims=0)
        else:
            assert_same_shape('quries', queries, 'keys', keys)
            assert_same_partial_shape('valid_lens', valid_lens, 'queries', queries, dims=(0, 1))
...
class EncoderBlock(nn.Module):
    ...
    def forward(
        ...
        # Ensure the same batch size
        assert_same_partial_shape('X', X, 'valid_lens', valid_lens, dims=0)
...
class Encoder(nn.Module):
    ...
    def forward(
        ...
        assert_dimension('valid_lens', valid_lens, 1)
        # Ensure the same batch size
        assert_same_partial_shape('source', source, 'valid_lens', valid_lens, dims=0)
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        assert_same_partial_shape('X', X, 'encoder_output', encoder_output, dims=(0, 2))
        assert_dimension_size('X', X, 2, self._num_hidden_units)
        ...
        outputs, weights = self._causal_mha(X, X, X, decoder_valid_lens)
        assert_shape('outputs', outputs, outputs_shape)
        outputs = self._addnorm1(X, outputs)
        assert_shape('outputs', outputs, outputs_shape)
...
class Decoder(nn.Module):
    ...
    def forward(
        ...
        assert_dimension('encoder_valid_lens', encoder_valid_lens, 1)
        # Ensure the same batch size
        assert_same_partial_shape('X', X, 'encoder_outputs', encoder_outputs, dims=0)
        assert_same_partial_shape(
            'encoder_outputs', encoder_outputs, 'encoder_valid_lens', encoder_valid_lens, dims=0)
...
class TransformerTranslator(nn.Module):
    ...
    def forward(
        ...
        # Ensure the same batch size
        assert_same_partial_shape('source', source, 'source_valid_lens', source_valid_lens, dims=0)
        assert_same_partial_shape('source', source, 'target_X', target_X, dims=0)
    ...
    def predict(
        ...
        - value #1: the predicted target sequence.
        - value #2: a tuple of three tensors of attention weights, in the following order:
    ...
    def _predict(
        ...
        attention_weights = (self._encoder.attention_weights, *self._decoder.attention_weights)

        return ''.join(lines_of_words[0]), attention_weights
...
if __name__ == "__main__":
    ...
    #     bleu = 1.00
    # prediction: #1
    #     source = 'the radio didn't mention the incident .'
    #     target = '電台沒有提到這個事故。'
    #     prediction = '收音機没有提到這件事'
    #     bleu = 0.49
    # prediction: #2
    #     source = 'i'm sorry , but i can't answer right away .'
    #     target = '我很抱歉，但我無法立即回答。'
    #     prediction = '對不起，但我無法回答'
    #     bleu = 0.48
    # prediction: #3
    #     source = 'i'd do anything for tom .'
    #     target = '我会为汤姆做任何事。'
    #     prediction = '我会为汤姆做任何事物'
    #     bleu = 0.92
    # epoch #29, train_loss = 0.483, validate_loss = 4.112, perplexity = 61.09, bleu_scores = ['1.00', '0.49', '0.48', '0.92']
    # device = cuda, elapsed time: 102.6 seconds
    # done!
