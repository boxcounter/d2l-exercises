...
class DenseBlock(nn.Module):
    def __init__(
        ...
        self._conv_blocks = [
            self._new_conv_block(growth_rate)
    ...
    def forward(
        ...
        # Insert X into the list to simplify the loop, which can be thought of
        # as the ouput from the previous block.
        intermediate_outputs = [X]
        concatenated_output = X
        for cb in self._conv_blocks:
            input_ = intermediate_outputs[-1]
            output = cb(input_)
...
class DenseNet(nn.Module):
    def __init__(
        ...
        self._net = nn.Sequential(
            self._new_stem(),
            self._new_body(num_conv_blocks, growth_rate),
    ...
    def _new_body(
        ...
        assert len(num_conv_blocks) > 0

        net = nn.Sequential()

        out_channels = growth_rate
        for num in num_conv_blocks[:-1]:
            net.append(DenseBlock(num_conv_blocks=num, growth_rate=growth_rate))
            out_channels += num * growth_rate
            net.append(TransitionBlock(out_channels=out_channels//2))

        # The final dense block, requires no transition block after it.
        net.append(
            DenseBlock(num_conv_blocks=num_conv_blocks[-1], growth_rate=growth_rate)
            )
...
def main(
    ...
    plotter = MetricsPlotter(
        title=f"FashionMNIST Classifier ({DenseNet.__name__})",
...
if __name__ == "__main__":
    ...
    # Final output:
    # epoch #9, train_loss = 0.333, validate_loss = 0.364, accuracy = 87.2%
