...
# Type alias
BlockParam = tuple[ConvParam, ConvParam, ConvParam]
...
class NiN(nn.Module):
    def __init__(
        ...
        max_pool_params: list[PoolParam],
        dropout_probability: float,
        final_block_param: BlockParam,
        global_avg_pool_shape: tuple[int, int],
    ) -> None:
        ...
        # NiN blocks
        for bp, pp in zip(block_params, max_pool_params):
            self._net.append(self._new_block(bp))
            self._net.append(nn.MaxPool2d(**dataclasses.asdict(pp)))

        # Dropout layer
        self._net.append(nn.Dropout(dropout_probability))

        # Final NiN block
        self._net.append(self._new_block(final_block_param))

        # Global AvgPool
        self._net.append(nn.AdaptiveAvgPool2d(global_avg_pool_shape))
    ...
    def _new_block(
        ...
        block = nn.Sequential()
        for cp in block_param:
            block.append(nn.LazyConv2d(**dataclasses.asdict(cp)))
...
def main(
    ...
    model = NiN(
        ...
        block_params=[
            (
                ConvParam(out_channels=96, kernel_size=11, stride=4),
                ConvParam(out_channels=96, kernel_size=1),
                ConvParam(out_channels=96, kernel_size=1),
            ),
            (
                ConvParam(out_channels=256, kernel_size=5, stride=1, padding=2),
                ConvParam(out_channels=256, kernel_size=1),
                ConvParam(out_channels=256, kernel_size=1),
            ),
            (
                ConvParam(out_channels=384, kernel_size=3, stride=1, padding=1),
                ConvParam(out_channels=384, kernel_size=1),
                ConvParam(out_channels=384, kernel_size=1),
            ),
        ],
        max_pool_params=[
            PoolParam(kernel_size=3, stride=2),
            PoolParam(kernel_size=3, stride=2),
            PoolParam(kernel_size=3, stride=2),
        ],
        dropout_probability=0.5,
        final_block_param=(
            ConvParam(out_channels=10, kernel_size=3, stride=1, padding=1),
            ConvParam(out_channels=10, kernel_size=1),
            ConvParam(out_channels=10, kernel_size=1),
        ),
        global_avg_pool_shape=(1, 1),
    )
    loss_measurer = nn.CrossEntropyLoss()
    ...
    plotter = MetricsPlotter(
        title="FashionMNIST Classifier (NiN)", filename="metrics.png")
...
if __name__ == "__main__":
    ...
    # Final output:
    # epoch #29, train_loss = 0.536, validate_loss = 0.549, accuracy = 81.0%
    # elapsed time: 806.0 seconds
    # done!
