...
def forward_hook(
    ...
    assert hasattr(module, 'v_cls_type') and isinstance(module, module.v_cls_type), \
        f"Invalid module type (expected '{module.v_cls_type}', got {type(module)})"
    assert isinstance(output, torch.Tensor), \
        f"Invalid output type (expected 'torch.Tensor', got {type(output)})"
    assert hasattr(module, 'v_name'), \
        f"Missing 'v_name' attribute in module {module}"
    assert hasattr(module, 'v_interval'), \
        f"Missing 'v_interval' attribute in module {module}"
    assert hasattr(module, 'v_num_forward'), \
        f"Missing 'v_num_forward' attribute in module {module}"

    module.v_num_forward += 1
    if module.v_num_forward % module.v_interval == 0:
        logger.debug("{}'s output: mean = {:.1e}, median = {:.1e}, std = {:.1e}",
                     module.v_name, output.mean(), output.median(), output.std())
        W = module.weight
        logger.debug("{}'s weight: mean = {:.1e}, median = {:.1e}, std = {:.1e}",
                     module.v_name, W.mean(), W.median(), W.std())
...
def backward_hook(
    ...
    assert isinstance(grad_output, tuple), \
        f"Invalid grad_output type (expected 'tuple', got {type(grad_output)})"
    assert hasattr(module, 'v_name'), \
        f"Missing 'v_name' attribute in module {module}"
    assert hasattr(module, 'v_interval'), \
        f"Missing 'v_interval' attribute in module {module}"
    assert hasattr(module, 'v_num_backward'), \
        f"Missing 'v_num_backward' attribute in module {module}"

    module.v_num_backward += 1
    if module.v_num_backward % module.v_interval == 0:
        grad = grad_output[0]
        logger.debug("{}'s grad: mean = {:.1e}, median = {:.1e}, std = {:.1e}",
                     module.v_name, grad.mean(), grad.median(), grad.std())


class VerboseLazyConv2d(nn.LazyConv2d):
    instance_count = 0
    cls_type = nn.Conv2d

    def __init__(self, *args, interval=1_000, **kwargs):
        super().__init__(*args, **kwargs)
        VerboseLazyConv2d.instance_count += 1
        self.v_cls_type = nn.Conv2d
        self.v_index = VerboseLazyConv2d.instance_count
        self.v_name = f"conv2d-{self.v_index}"
        self.v_interval = interval
        self.v_num_forward = 0
        self.v_num_backward = 0
        self.register_forward_hook(forward_hook)
        self.register_full_backward_hook(backward_hook)


class VerboseLazyLinear(nn.LazyLinear):
    instance_count = 0
    cls_type = nn.Linear

    def __init__(self, *args, interval=1_000, **kwargs):
        super().__init__(*args, **kwargs)
        VerboseLazyLinear.instance_count += 1
        self.v_cls_type = nn.Linear
        self.v_index = VerboseLazyLinear.instance_count
        self.v_name = f"linear-{self.v_index}"
        self.v_interval = interval
        self.v_num_forward = 0
        self.v_num_backward = 0
        self.register_forward_hook(forward_hook)
...
class BNLeNet(nn.Module):
    def __init__(
        ...
        for cp, pp in conv_pool_params:
            self._net.append(VerboseLazyConv2d(**dataclasses.asdict(cp)))
            self._net.append(nn.LazyBatchNorm2d())
        ...
        for n in num_linear_units:
            self._net.append(VerboseLazyLinear(n))
            self._net.append(nn.ReLU())
...
def main(
    ...
    plotter = MetricsPlotter(
        title=f"FashionMNIST Classifier ({BNLeNet.__name__})", filename="metrics.png")
...
if __name__ == "__main__":
    ...
    # Final output:
    # epoch #19, train_loss = 0.226, validate_loss = 0.300, accuracy = 89.2%
    # elapsed time: 179.7 seconds
    # done!
