...
class InceptionBlock(nn.Module):
    def __init__(
        ...
        assert all((isinstance(out_channels, tuple) and len(out_channels) == 4,
                    isinstance(out_channels[0], int),
                    isinstance(out_channels[1], tuple) and len(out_channels[1]) == 2,
                    isinstance(out_channels[2], tuple) and len(out_channels[2]) == 2,
                    isinstance(out_channels[3], int)
                    )), "Invalid out_channels"
        super().__init__()

        self._branch_1 = nn.Sequential(
            nn.LazyConv2d(out_channels=out_channels[0], kernel_size=1),
            nn.ReLU(),
            )
        self._branch_2 = nn.Sequential(
            nn.LazyConv2d(out_channels=out_channels[1][0], kernel_size=1),
            nn.ReLU(),
            nn.LazyConv2d(out_channels=out_channels[1][1], kernel_size=3, padding=1),
            nn.ReLU(),
            )
        self._branch_3 = nn.Sequential(
            nn.LazyConv2d(out_channels=out_channels[2][0], kernel_size=1),
            nn.ReLU(),
            nn.LazyConv2d(out_channels=out_channels[2][1], kernel_size=5, padding=2),
            nn.ReLU(),
        )
        self._branch_4 = nn.Sequential(
            nn.MaxPool2d(kernel_size=3, stride=1, padding=1),
            nn.LazyConv2d(out_channels=out_channels[3], kernel_size=1),
            nn.ReLU(),
        )
    ...
    def forward(
        ...
        o1 = self._branch_1(X) # o1.shape = batch_size x out_channels x height x width
        o2 = self._branch_2(X)
        o3 = self._branch_3(X)
        o4 = self._branch_4(X)
        return torch.cat((o1, o2, o3, o4), dim=1) # concatenate along the channel dimension
...
class GoogLeNet(nn.Module):
    def __init__(
        ...
        self._net = nn.Sequential(
            ...
            # First 2 inception blocks
            InceptionBlock(out_channels=(64, (96, 128), (16, 32), 32)),
            InceptionBlock(out_channels=(128, (128, 192), (32, 96), 64)),

            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),

            # Second 5 inception blocks
            InceptionBlock(out_channels=(192, (96, 208), (16, 48), 64)),
            InceptionBlock(out_channels=(160, (112, 224), (24, 64), 64)),
            InceptionBlock(out_channels=(128, (128, 256), (24, 64), 64)),
            InceptionBlock(out_channels=(112, (144, 288), (32, 64), 64)),
            InceptionBlock(out_channels=(256, (160, 320), (32, 128), 128)),

            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),

            # Third 2 inception blocks
            InceptionBlock(out_channels=(256, (160, 320), (32, 128), 128)),
            InceptionBlock(out_channels=(384, (192, 384), (48, 128), 128)),
...
def main(
    ...
    model = GoogLeNet(input_shape=input_shape, num_labels=10)
    loss_measurer = nn.CrossEntropyLoss()
    ...
    plotter = MetricsPlotter(
        title=f"FashionMNIST Classifier ({GoogLeNet.__name__})", filename="metrics.png")
...
if __name__ == "__main__":
    ...
    # Final output:
    # epoch #19, train_loss = 0.314, validate_loss = 0.338, accuracy = 86.9%
    # elapsed time: 981.3 seconds
    # done!
