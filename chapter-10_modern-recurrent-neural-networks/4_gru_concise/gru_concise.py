...
class GRULMConcise(nn.Module):
    def __init__(
        ...
        self._mem_cell = nn.GRU(
            vocab.size, num_hidden_units, self._num_layers, batch_first=True, device=device)
...
def main(
    ...
    model = GRULMConcise(dataset.vocab, num_hidden_units)
    loss_measurer = nn.CrossEntropyLoss()
    ...
    plotter.plot(title=f"Language Model ({GRULMConcise.__name__})", filename="metrics.jpg")
...
if __name__ == "__main__":
    ...
    # Final output:
    # epoch #19, train_loss = 1.519, validate_loss = 1.816, accuracy = 46.2%, perplexity = 6.14
    # device = cuda, elapsed time: 23.2 seconds
    # prediction for 'it has': ' i saw the same and '
    # done!
