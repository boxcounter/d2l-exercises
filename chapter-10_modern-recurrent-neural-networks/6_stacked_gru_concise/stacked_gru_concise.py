...
class StackedGRULMConcise(nn.Module):
    def __init__(
        self,
        vocab: Vocabulary,
        num_layers: int,
        ...
        self._mem_cells = nn.GRU(
            vocab.size, num_hidden_units, num_layers, batch_first=True, device=device)
    ...
    def _forward(
        ...
        assert_shape('X', X, (batch_size, num_steps, self._vocab.size))

        output, H = self._mem_cells(X, H_prev)
        assert_shape('output', output, (batch_size, num_steps, self._num_hidden_units))
        assert_shape('H', H, (self._num_layers, batch_size, self._num_hidden_units))

        output = self._output_layer(output)
        assert_shape('output', output, (batch_size, num_steps, self._vocab.size))
        return output, H
...
def main(
    ...
    model = StackedGRULMConcise(dataset.vocab, num_layers, num_hidden_units)
    loss_measurer = nn.CrossEntropyLoss()
    ...
    logger.info("num_layers = {}, num_hidden_units = {}, num_steps = {}, device = {}",
                num_layers, num_hidden_units, num_steps, device)
    ...
    plotter.plot(title=f"Language Model ({StackedGRULMConcise.__name__})", filename="metrics.jpg")
...
if __name__ == "__main__":
    ...
    # Final output:
    # epoch #19, train_loss = 1.400, validate_loss = 1.726, accuracy = 48.9%, perplexity = 5.62
    # device = cuda, elapsed time: 24.6 seconds
    # prediction for 'it has': ' even and so in a st'
    # done!
