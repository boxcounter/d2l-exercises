...
class MemoryCell(nn.Module):
    def __init__(
        ...
        self._reset_gate = Gate(num_features, num_hidden_units, std)
        self._update_gate = Gate(num_features, num_hidden_units, std)
        self._candidate = Candidate(num_features, num_hidden_units, std)
...
class StackedGRULMScratch(nn.Module):
    def __init__(
        self,
        vocab: Vocabulary,
        num_layers: int,
        ...
        # The memory cells (hidden layers)
        self._mem_cells = nn.Sequential()
        for layer in range(num_layers):
            num_features = vocab.size if layer == 0 else num_hidden_units
            self._mem_cells.append(MemoryCell(num_features, num_hidden_units))
    ...
    def predict(
        ...
        H = None # hidden state
        vocab_size, num_layers, num_hidden_units = (
            self._vocab.size, self._num_layers, self._num_hidden_units)
    ...
    def _forward(
        ...
        if H_prev is None:
            H_prev_step = [None] * self._num_layers
        else:
            assert_shape('H_prev', H_prev, (self._num_layers, batch_size, self._num_hidden_units))
            H_prev_step = H_prev
        ...
        outputs = [] # outputs of all the time steps
        Hs = [] # hidden states of all the layers at the final time step
        for step, X_ in enumerate(X_multi_steps):
            H = X_
            for layer in range(self._num_layers):
                H = self._mem_cells[layer](H, H_prev_step[layer])
                assert_shape('H', H, (batch_size, self._num_hidden_units))

                H_prev_step[layer] = H

                if step == num_steps - 1: # at the final time step
                    Hs.append(H)
        ...
        Hs = torch.stack(Hs, 0)
        assert_shape('Hs', Hs, (self._num_layers, batch_size, self._num_hidden_units))
...
def main(
    ...
    model = StackedGRULMScratch(dataset.vocab, num_layers, num_hidden_units)
    loss_measurer = nn.CrossEntropyLoss()
    ...
    plotter.plot(title=f"Language Model ({StackedGRULMScratch.__name__})", filename="metrics.jpg")
...
if __name__ == "__main__":
    ...
    # Final output:
    # epoch #19, train_loss = 1.573, validate_loss = 1.810, accuracy = 46.6%, perplexity = 6.11
    # device = cuda, elapsed time: 144.6 seconds
    # prediction for 'it has': ' a conting and i had'
    # done!
