...
class Component(nn.Module):
    def __init__(
        ...
        if activation == 'sigmoid':
            self.activation_fn = Func.sigmoid
        elif activation == 'tanh':
            self.activation_fn = Func.tanh
        else:
    ...
    def forward(
        ...
        return self.activation_fn(X @ self._inputs_weight +
                                  hidden_state @ self._hidden_weight +
                                  self._bias)
...
class MemoryCell(nn.Module):
    def __init__(
        ...
        self._forget_gate = Component(vocab.size, num_hidden_units, std)
        self._input_gate = Component(vocab.size, num_hidden_units, std)
        self._input_node = Component(vocab.size, num_hidden_units, std, 'tanh')
        self._output_gate = Component(vocab.size, num_hidden_units, std)
    ...
    def forward(
        ...
        - item #0: the internal state tensor with the shape of (batch_size, num_hidden_units)
        - item #1: the hidden state tensor with the shape of (batch_size, num_hidden_units)
        """

        state_size = (X.shape[0], self._num_hidden_units)
        # the cell/internal state of the memory cell
        C_prev = (torch.zeros(state_size, device=device)
                  if internal_state is None
                  else internal_state)
        # the hidden state of the memory cell at the last timestep
        H_prev = (torch.zeros(state_size, device=device)
                  if hidden_state is None
                  else hidden_state)

        F = self._forget_gate(X, H_prev)
        I = self._input_gate(X, H_prev) # noqa: E741 # disable 'ambiguous-variable-name' error
        C_tilde = self._input_node(X, H_prev)
        C = F * C_prev + I * C_tilde

        O = self._output_gate(X, H_prev) # noqa: E741
        H = O * Func.tanh(C)

        return C, H


class LSTMLMScratch(nn.Module):
    def __init__(
        self,
        vocab: Vocabulary,
        num_hidden_units: int,
    ...
    def _forward(
        ...
        C, H = internal_state, hidden_states
        outputs = []
...
def main(
    ...
    model = LSTMLMScratch(dataset.vocab, num_hidden_units)
    loss_measurer = nn.CrossEntropyLoss()
    ...
    plotter.plot(title=f"Language Model ({LSTMLMScratch.__name__})", filename="metrics.jpg")
