...
class BidirectionalGRULMScratch(nn.Module):
    def __init__(
        self,
        vocab: Vocabulary,
        num_hidden_units: int,
        ...
        # The memory cells
        self._mem_cell_forward = MemoryCell(vocab, num_hidden_units)
        self._mem_cell_backward = MemoryCell(vocab, num_hidden_units)

        # Parameters of the output layer
        self._W_o = nn.Parameter(
            torch.randn((2*num_hidden_units, vocab.size), device=device) * std)
    ...
    def predict(
        ...
        for _ in range(num_prediction):
            ...
            outputs, Hs = self._forward(X, Hs)
            assert_shape('outputs', outputs, (batch_size, num_steps, vocab_size))
            assert Hs is not None and len(Hs) == 2
            assert_shape('Hs[0]', Hs[0], (batch_size, num_hidden_units))
            assert_shape('Hs[1]', Hs[1], (batch_size, num_hidden_units))
    ...
    def _forward(
        self,
        X: torch.Tensor,
        Hs_prev: tuple[torch.Tensor, torch.Tensor] | None = None,
    ) -> tuple[torch.Tensor, tuple[torch.Tensor, torch.Tensor] | None]:
        """
        Parameters:
        - X: the input tensor with the shape of (batch_size, num_steps)
        - H_prev: a tuple of two hidden state tensors of the previous time step:
            - item #0: the forward hidden state tensor with the shape of
                (batch_size, num_hidden_units)
            - item #1: the backward hidden state tensor with the shape of
                (batch_size, num_hidden_units)

        Returns a tuple of two tensors:
        - item #0: the output tensor with the shape of (batch_size, num_steps, vocab_size)
        - item #1: a tuple of two hidden state tensors of current time step:
            - item #0: the forward hidden state tensor with the shape of
                (batch_size, num_hidden_units)
            - item #1: the backward hidden state tensor with the shape of
                (batch_size, num_hidden_units)
        ...
        H_forward, H_backward = (None, None) if Hs_prev is None else Hs_prev
        outputs = []
        for step in range(num_steps):
            X_forward = X_multi_steps[step]
            H_forward = self._mem_cell_forward(X_forward, H_forward)
            assert_shape('H_forward', H_forward, (batch_size, self._num_hidden_units))

            X_backward = X_multi_steps[num_steps - step - 1]
            H_backward = self._mem_cell_backward(X_backward, H_backward)
            assert_shape('H_backward', H_backward, (batch_size, self._num_hidden_units))
        ...
        assert H_forward is not None
        assert H_backward is not None
...
class Trainer:
    def __init__(
        self,
        model: BidirectionalGRULMScratch,
...
def main(
    ...
    model = BidirectionalGRULMScratch(dataset.vocab, num_hidden_units)
    loss_measurer = nn.CrossEntropyLoss()
    ...
    plotter.plot(
        title=f"Language Model ({BidirectionalGRULMScratch.__name__})", filename="metrics.jpg")
...
if __name__ == "__main__":
    ...
    # Final output:
    # epoch #19, train_loss = 1.554, validate_loss = 1.789, accuracy = 46.8%, perplexity = 5.9
    # device = cuda, elapsed time: 109.4 seconds
    # prediction for 'it has': ' a strange and the m'
    # done!
