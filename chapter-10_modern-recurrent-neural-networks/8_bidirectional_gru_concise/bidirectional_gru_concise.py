...
class BidirectionalGRULMConcise(nn.Module):
    def __init__(
        self,
        vocab: Vocabulary,
        ...
        self._mem_cell = nn.GRU(
            vocab.size, num_hidden_units, self._num_layers,
            batch_first=True, bidirectional=(self._num_directions==2), device=device)
    ...
    def predict(
        ...
        H = None # hidden state
        num_layers, num_directions, vocab_size, num_hidden_units = (
            self._num_layers, self._num_directions, self._vocab.size, self._num_hidden_units)
    ...
    def _forward(
        ...
        X = self._vocab.one_hot_encode(X)
        assert_shape('X', X, (batch_size, num_steps, vocab_size))

        output, H = self._mem_cell(X, H_prev)
        assert_shape('output', output, (batch_size, num_steps, num_directions * num_hidden_units))
        assert_shape('H', H, (num_directions * num_layers, batch_size, num_hidden_units))

        output = self._output_layer(output)
        assert_shape('output', output, (batch_size, num_steps, vocab_size))
        return output, H
...
class Trainer:
    def __init__(
        self,
        model: BidirectionalGRULMConcise,
    ...
    def fit(self) -> float:
        ...
        self._model.train()
        for X, y in self._dataset.get_data_loader(train=True, shuffle=False):
            X = X.to(device)
...
class Validator:
    ...
    def __call__(self) -> tuple[float, float, float]:
        ...
        with torch.inference_mode():
            for X, y in self._dataset.get_data_loader(train=False, shuffle=True):
                X = X.to(device)
...
def main(
    ...
    model = BidirectionalGRULMConcise(dataset.vocab, num_hidden_units)
    loss_measurer = nn.CrossEntropyLoss()
    ...
    plotter.plot(
        title=f"Language Model ({BidirectionalGRULMConcise.__name__})", filename="metrics.jpg")
...
if __name__ == "__main__":
    ...
    # Final output:
    # epoch #9, train_loss = 0.072, validate_loss = 0.076, accuracy = 97.8%, perplexity = 1.08
