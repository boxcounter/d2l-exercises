...
class RNNLMScratch(nn.Module):
    def __init__(
        ...
        self._W_xh = nn.Parameter(
            torch.randn((vocab.size, num_steps), device=device) * std)
        self._W_hh = nn.Parameter(
            torch.randn((num_steps, num_steps), device=device) * std)
        self._B_xh = nn.Parameter(
            torch.zeros((num_steps), device=device))

        # Parameters of the output layer
        self._W_hq = nn.Parameter(
            torch.randn((num_steps, vocab.size), device=device) * std)
    ...
    def predict(
        ...
        for _ in range(num_prediction):
            ...
            assert_shape("hidden_states", hidden_states, (1, self._num_steps))
    ...
    def _forward(
        ...
        if hidden_states is None:
            H = torch.zeros((batch_size, self._num_steps), device=device)
        else:
            H = hidden_states
        ...
        for X in X_multi_steps:
            ...
            assert_shape("H", H, (batch_size, self._num_steps))
...
class Trainer:
    ...
    def fit(self) -> float:
        ...
        for X, y in self._dataset.get_data_loader(train=True):
            ...
            y_pred_one_hot = self._model(X)
            loss = self._loss_measurer(y_pred_one_hot, y_one_hot)
    ...
    def __call__(self) -> tuple[float, float, float]:
        ...
        with torch.inference_mode():
            for X, y in self._dataset.get_data_loader(train=False):
                ...
                total_loss += self._loss_measurer(y_pred_one_hot, y_one_hot).item()
                num_batches += 1

                y_pred_indices = self.index(y_pred_one_hot).flatten()
...
def main(
    ...
    model = RNNLMScratch(vocab=dataset.vocab, num_steps=num_steps)
    loss_measurer = nn.CrossEntropyLoss()
    ...
    plotter.plot(title=f"Language Model ({RNNLMScratch.__name__})", filename="metrics.png")
...
if __name__ == "__main__":
    ...
    # Final output:
    # epoch #99, train_loss = 2.783, validate_loss = 2.949, accuracy = 30.3%, perplexity = 19.08
    # device = cuda, elapsed time: 160.9 seconds
    # prediction for 'it has': ' be he himb frce frc'
    # done!
